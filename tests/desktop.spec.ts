...
test.describe('desktop tests', () => {
  ...
  test('positive simple transfer test', async ({ page }) => {
    ...
    await expect(page.getByTestId('message-text'))
      .toHaveText(`Przelew wykonany! ${transferReceipient} - ${transferAmount},00PLN - ${transferTitle}`)
  });
  ...
  test('positive phone top-up test', async ({ page }) => {
    ...
    //Assert
    await expect(page.getByRole('dialog', { name: 'Doładowanie wykonane' })).toBeVisible();
    await page.getByTestId('close-button').click();

    await expect(page.getByTestId('message-text')).toHaveText(`Doładowanie wykonane! ${topupAmount},00PLN na numer ${topupReceiver}`);
  });
});